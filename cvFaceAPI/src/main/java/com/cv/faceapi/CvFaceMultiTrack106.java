...
import com.cv.faceapi.CvFaceApiBridge.cv_face_106_t;
import com.cv.faceapi.CvFaceApiBridge.cv_face_t;
import com.sun.jna.Pointer;
import com.sun.jna.ptr.IntByReference;
import com.sun.jna.ptr.PointerByReference;

public class CvFaceMultiTrack106 {
     ...
     * Note
        track only one face： 
        frist:trackHandle = CvFaceApiBridge.FACESDK_INSTANCE.cv_face_create_tracker_106(null, CV_FACE_SKIP_BELOW_THRESHOLD);
        second: setMaxDetectableFaces(1)参数设为1
     *  track多张人脸：	
     *  first：开启多张人脸检测(CV_TRACK_MULTI_TRACKING)
     *  trackHandle = CvFaceApiBridge.FACESDK_INSTANCE.cv_face_create_tracker_106(null, CV_FACE_SKIP_BELOW_THRESHOLD | CV_TRACK_MULTI_TRACKING);
        second:setMaxDetectableFaces(-1)参数设为-1
     */
     
    public CvFaceMultiTrack106() {
    	trackHandle = CvFaceApiBridge.FACESDK_INSTANCE.cv_face_create_tracker_106(null, CV_FACE_SKIP_BELOW_THRESHOLD | CV_TRACK_MULTI_TRACKING | CV_FACE_RESIZE_IMG_320W);
    }
    
    public int setMaxDetectableFaces(int max)
    {
    	int rst = CvFaceApiBridge.FACESDK_INSTANCE.cv_face_track_106_set_detect_face_cnt_limit(trackHandle, max);
        return rst;
    }
    ...
    protected void finalize() throws Throwable {
        ...
        CvFaceApiBridge.FACESDK_INSTANCE.cv_face_destroy_tracker_106(trackHandle);
     ...
     */
    public CvFace106[] track(Bitmap image, int orientation) {
     ...
     * @return CvFace array, each one in array is Detected by SDK native API
     */
    public CvFace106[] track(int[] colorImage,int cvImageFormat, int imageWidth, int imageHeight, int imageStride, int orientation) {
    	if(DEBUG)System.out.println("SampleLiveness-------->CvFaceMultiTrack--------->track2");
    	
        long startTime = System.currentTimeMillis();
        int rst = CvFaceApiBridge.FACESDK_INSTANCE.cv_face_track_106(trackHandle, colorImage, cvImageFormat,imageWidth,
        ...
        if (ptrToSize.getValue() == 0) {
            ...
            return new CvFace106[0];
        }

        cv_face_106_t arrayRef = new cv_face_106_t(ptrToArray.getValue());
        arrayRef.read();
        cv_face_106_t[] array = cv_face_106_t.arrayCopy((cv_face_106_t[]) arrayRef.toArray(ptrToSize.getValue()));
        CvFaceApiBridge.FACESDK_INSTANCE.cv_face_release_tracker_106_result(ptrToArray.getValue(), ptrToSize.getValue());
        
        CvFace106[] ret = new CvFace106[array.length]; 
        for (int i = 0; i < array.length; i++) {
        	ret[i] = new CvFace106(array[i]);
        }
     ...
     * @return CvFace array, each one in array is Detected by SDK native API
     */
    public CvFace106[] track(byte[] image, int orientation,int width,int height) {
    	if(DEBUG){
     ...
     * @return CvFace array, each one in array is Detected by SDK native API
     */
    public CvFace106[] track(byte[] colorImage,int cvImageFormat, int imageWidth, int imageHeight, int imageStride, int orientation) {
    	if(DEBUG){
